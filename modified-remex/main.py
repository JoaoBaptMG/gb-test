...
class AutotileExpander(Script):
    ...
    def _initializeLocations(self):
        ...
        self._minitilePositionGroup[0, 0], self._minitilePositionGroup[TS//2, 0], self._minitilePositionGroup[0, TS//2], self._minitilePositionGroup[TS//2, TS//2] = "NO", "NE", "SO", "SE"
        ...
        self._minitileTypeDependingOnGroup[TS, 0, "NO"] = "External angle"
        self._minitileTypeDependingOnGroup[TS, 0, "NE"] = "External angle"
        self._minitileTypeDependingOnGroup[TS, 0, "SO"] = "External angle"
        self._minitileTypeDependingOnGroup[TS, 0, "SE"] = "External angle"
        self._minitileTypeDependingOnGroup[0, TS, "NO"] = "Internal angle"
        self._minitileTypeDependingOnGroup[TS, TS, "NE"] = "Internal angle"
        self._minitileTypeDependingOnGroup[0, TS*2, "SO"] = "Internal angle"
        self._minitileTypeDependingOnGroup[TS, TS*2, "SE"] = "Internal angle"
        self._minitileTypeDependingOnGroup[TS, TS, "NO"] = "HorizontalEdge"
        self._minitileTypeDependingOnGroup[0, TS, "NE"] = "HorizontalEdge"
        self._minitileTypeDependingOnGroup[TS, TS*2, "SO"] = "HorizontalEdge"
        self._minitileTypeDependingOnGroup[0, TS*2, "SE"] = "HorizontalEdge"
        self._minitileTypeDependingOnGroup[0, TS*2, "NO"] = "VerticalEdge"
        self._minitileTypeDependingOnGroup[TS, TS*2, "NE"] = "VerticalEdge"
        self._minitileTypeDependingOnGroup[0, TS, "SO"] = "VerticalEdge"
        self._minitileTypeDependingOnGroup[TS, TS, "SE"] = "VerticalEdge"
        self._minitileTypeDependingOnGroup[TS, TS*2, "NO"] = "Normal"
        self._minitileTypeDependingOnGroup[0, TS*2, "NE"] = "Normal"
        self._minitileTypeDependingOnGroup[0, TS, "SE"] = "Normal"
        self._minitileTypeDependingOnGroup[TS, TS, "SO"] = "Normal"

    def _makeAutotile(self, *types):
        autotileSurface = ImagePIL.new("RGB", (TS,TS))
        minitileAbs, minitileOrd, i = 0, 0, 0
        while minitileOrd < TS:
            minitileAbs = 0
            while minitileAbs < TS:
    ...
    def expandAutotile(self, autotileFilename):
        ...
        while groupOrd < self._imageAutotile.size[1]: #We run through the groups of minitiles
            ...
            while groupAbs < self._imageAutotile.size[0]:
                ...
                while minitileOrd < TS: #We run through the minitiles
                    minitileAbs = 0
                    while minitileAbs < TS:
                        minitilePosition = self._minitilePositionGroup[minitileAbs, minitileOrd]
                        minitileType = self._minitileTypeDependingOnGroup[groupAbs, groupOrd, minitilePosition]
                        self._minitiles[minitilePosition][minitileType] = self._imageAutotile.crop((groupAbs+minitileAbs, groupOrd+minitileOrd, groupAbs+minitileAbs+TS//2, groupOrd+minitileOrd+TS//2))
                        minitileAbs += TS//2
                    minitileOrd += TS//2
                groupAbs += TS
            groupOrd += TS
        ...
        autotilesSurfaces[42] = self._makeAutotile("Internal angle", "Internal angle", "VerticalEdge", "VerticalEdge")
        autotilesSurfaces[43] = self._makeAutotile("Internal angle", "HorizontalEdge", "Internal angle", "HorizontalEdge")
        autotilesSurfaces[44] = self._makeAutotile("VerticalEdge", "VerticalEdge", "Internal angle", "Internal angle")
        autotilesSurfaces[45] = self._makeAutotile("HorizontalEdge", "Internal angle", "HorizontalEdge", "Internal angle")
        autotilesSurfaces[46] = self._makeAutotile("Internal angle", "Internal angle", "Internal angle", "Internal angle")
        autotilesSurfaces[47] = self._makeAutotile("Internal angle", "Internal angle", "Internal angle", "Internal angle")
        ##### We make the final surface by blitting our individual surfaces
        autotileAbs, autotileOrd, i, self._expandedAutotile = 0, 0, 0, ImagePIL.new("RGB", (TS*8, TS*6))
        while autotileOrd < TS * 6:
            autotileAbs = 0
            while autotileAbs < TS * 8:
    ...
    def _checkInputSize(self):
            image = ImagePIL.open(self._inputFilename)
            if image.size != (TS*2, TS*3):
                print("The input autotile \"{0}\" does not have the right size.\nIt must be {1} * {2} pixels wide. Please refer to tileA2 formatting from RPG Maker VX / VX Ace.".format(self._inputFilename, TS*2, TS*3))
                raise SystemExit
...
class TilesetGenerator(Script):
    ...
    def _checkInputSize(self):
            image = ImagePIL.open(self._inputFilename)
            if image.size != (TS*8, TS*6):
                print("The input expanded autotile \"{0}\" does not have the right size.\nIt must be {1} * {2} pixels wide. Please expand an autotile from RPG Maker VX / VX Ace with this program first.".format(self._inputFilename, TS*8, TS*6))
                raise SystemExit
    ...
    def makeXML(self, inputFilename, outputFilename="Tileset"):
        ...
        tilesetXML.setAttribute("tilewidth", str(TS))
        tilesetXML.setAttribute("tileheight", str(TS))
        imageXML = mainXML.createElement("image")
        imageXML.setAttribute("source", self._inputFilename)
        imageXML.setAttribute("trans", "ff0000")
        width, height = TS*8, TS*6
...
class RuleMaker(Script):
    ...
    def _loadTileset(self):
        ...
        self._tilesetRegionsXML.setAttribute("tilewidth", str(TS))
        self._tilesetRegionsXML.setAttribute("tileheight", str(TS))
        self._tilesetRegionsXML.setAttribute("firstgid", "49")
        imageTilesetRegionsXML = self._tilesetRegionsConfig.createElement("image")
        imageTilesetRegionsXML.setAttribute("width",str(TS))
        imageTilesetRegionsXML.setAttribute("height",str(TS))
        imageTilesetRegionsXML.setAttribute("trans","ff0000")
        ...
        self._ruleXML.setAttribute("tilewidth", str(TS))
        self._ruleXML.setAttribute("tileheight", str(TS))
    ...
    def makeRule(self):
        ...
        propertyNames = ["MatchOutsideMap", "OverflowBorder"]
        properties = self._ruleConfig.createElement("properties")
        for prop in propertyNames:
            property = self._ruleConfig.createElement("property")
            property.setAttribute("name", prop)
            property.setAttribute("type", "bool")
            property.setAttribute("value", "true")
            properties.appendChild(property)
        self._ruleXML.appendChild(properties)
        i = 0
        while i < len(layers):
    ...
    def createRegionsImage(self, newRegionsFile=""):
        ...
        try:
            automappingImage = ImagePIL.new("RGB", (TS, TS), (255, 255, 255))
...
if __name__ == "__main__":
    ...
    expandSubCommand.add_argument("inputAutotile", help=f"The autotile to expand. It must follow a few rules. It must be a PNG image, {TS*2} * {TS*3} wide. It must use RPG Maker VX or VX Ace's TileA2 formatting.")
    ...
    makeTilesetSubCommand.add_argument("inputExpandedAutotile", help=f"The expanded autotile to make a tileset with. It must be a PNG image, {TS*8} * {TS*6} wide. To get this expanded autotile, use the autotile expander featured with Remex (with the command \"expand\").")
